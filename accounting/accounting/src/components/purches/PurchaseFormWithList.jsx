    ...
    try {
      for (const p of selected) {
        const stockEntry = {
          ...
          reference: `رسید خرید ${p.factorNo || p.PurchaseId}`,
          description: `ورود کالا بابت خرید ${p.ItemName} - فاکتور ${p.factorNo || "?"}`
        };
        ...
        const inventoryRes = await fetch("http://localhost:5000/api/inventory/increase", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            itemCode: p.ItemCode,
        ...
        if (!inventoryRes.ok) {
          ...
          console.warn("⚠️ خطا در افزایش موجودی:", errText);
        }
      }
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      <GenericForm
        ...
        fields={[
          ...
          { name: "ItemName", label: "نام کالا", type: "text", disabled: true },
          { name: "ItemSpec", label: "مشخصات کالا", type: "text", disabled: true },
          {
          ...
          { name: "SupplierName", label: "نام فروشنده", type: "text", disabled: true },
          { name: "Quantity", label: "تعداد", type: "number", required: true },
          { name: "UnitPrice", label: "قیمت واحد", type: "number", required: true },
          { name: "factorNo", label: "شماره فاکتور", type: "text", required: true },
          { name: "PurchaseDate", label: "تاریخ خرید", type: "date", required: true },
          { name: "Description", label: "شرح مختصر", type: "textarea" }
        ]}
        ...
        <table className="min-w-full text-sm text-right bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-100">
          <thead className="bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-100">
            <tr>
              ...
              <th className="px-4 py-2 border-b">شماره فاکتور</th>
          ...
          <tbody>
            {purchases.map((p) => (
              <tr key={p.PurchaseId} className="hover:bg-blue-50 dark:hover:bg-blue-800 transition">
                ...
                <td className="px-4 py-2 border-b">{p.factorNo || "-"}</td>
