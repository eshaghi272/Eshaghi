...
export const submitSaleTransaction = (req, res) => {
  const {
    ...
    description, factorNo, invoiceId = null, warehouseId = 1
  } = req.body;
    ...
    const trx = db.transaction(() => {
      ...
      const saleStmt = db.prepare(`
        INSERT INTO tblSale (
          ItemCode, ItemName, ItemSpec,
          CustomerName, CustomerNationalCode,
          Quantity, UnitPrice, SaleDate, Description, factorNo
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      // 2️⃣ ثبت تراکنش انبار
      const stockStmt = db.prepare(`
        INSERT INTO tblStockTransaction (
          transType, transDate, itemCode,
          warehouseId, quantity, reference, description, unitPrice, factorNo
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      // 4️⃣ صدور سند حسابداری فروش
      const amount = Number(quantity) * Number(unitPrice);
      const debitAcc = getAccountSetting('saleDebitAccount');     // حساب مشتریان
      const creditAcc = getAccountSetting('saleCreditAccount');   // حساب درآمد
      ...
      createAccountingEntry({
        ...
        description: `فروش ${itemName} به ${customerName}`,
        sourceTable: 'tblSale',
        sourceId: saleId,
        entries: [
          {
            ...
            DebitAmount: amount,
            CreditAmount: 0,
            SubsidiaryId: customerNationalCode || null
          },
          {
      ...
      console.log(`✅ فروش ثبت شد → SaleId ${saleId}`);
    });

    trx();
    res.json({ success: true, message: "✅ فروش با موفقیت ثبت شد و سند حسابداری صادر شد" });
  } catch (err) {
    console.error("❌ خطا در ثبت فروش:", err.message);
    res.status(500).json({ error: "خطا در ثبت فروش: " + err.message });
  }
};
...
export const submitFullInvoice = (req, res) => {
  ...
  try {
    const trx = db.transaction(() => {
      ...
      const invoiceStmt = db.prepare(`
        INSERT INTO tblSalesInvoice (
          ...
          unit, itemCode, itemName, itemSpec, quantity, unitPrice, factorNo
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);
      ...
      for (const line of lines) {
        ...
        saleStmt.run(
          ...
          line.factorNo || invoice.factorNo
        );
    ...
    console.error("❌ خطا در ثبت فاکتور کامل:", err.message);
    res.status(500).json({ error: "خطا در ثبت فاکتور کامل: " + err.message });
  }
};
/**
