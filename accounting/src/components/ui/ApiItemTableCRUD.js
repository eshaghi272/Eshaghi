...
export default function ApiItemTableCRUD({
    ...
    const [sortField, setSortField] = useState(columns[0]?.key || "itemName");
    ...
    const validateNewRow = () => {
        // اگر required خواستی، می‌تونی در columns [{key, label, required:true}] اضافه کنی
        for (const c of columns) {
            if (c.required && (newRow[c.key] === undefined || newRow[c.key] === "")) {
                return `فیلد "${c.label}" الزامی است`;
            }
        }
        return null;
    };
    ...
    return (
        <div className="max-w-6xl mx-auto p-6">
            <h2 className="text-2xl font-bold mb-4">جدول کالاها</h2>
