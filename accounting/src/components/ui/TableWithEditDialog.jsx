...
export default function TableWithEditDialog({
  apiUrl,
  columns,
  mapResponse,
  refreshTrigger
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(`خطا در حذف: ${response.status} - ${errorText}`);
      }
    } catch (err) {
      console.error("❌ خطا در حذف:", err);
      alert(`❌ خطا در حذف مورد: ${err.message}`);
    }
  };

  const handleSave = async (id, updatedData) => {
    try {
      // حذف فیلدهای نمایشی که با _ شروع می‌شوند
      ...
      throw err; // خطا را دوباره throw کن تا در EditModal هم catch شود
    }
  };
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
        ...
        {data.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 dark:text-gray-400">هنوز هیچ موردی ثبت نشده است.</p>
          </div>
        )}
      </div>
